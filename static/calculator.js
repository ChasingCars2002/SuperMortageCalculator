...
function calculateMortgage(inputs) {
  ...
  // Compute lifetime totals for tax, insurance, HOA, PMI
  const totalTax = +(monthlyTax * nPayments).toFixed(2);
    const totalInsurance = +(monthlyIns * nPayments).toFixed(2);
    const totalHoa = +(monthlyHoa * nPayments).toFixed(2);
    const totalPMI = +(monthlyPMI * nPayments).toFixed(2);

  const totalPaid = +(downPayment + totalPrincipal + totalInterest + totalTax + totalInsurance + totalHoa + totalPMI).toFixed(2);
